...
**Stream Deck GitHub Utilities** — a Stream Deck plugin that displays GitHub repository stats, CI/workflow status, PRs, issues, releases, and other GitHub information on Stream Deck devices.

- **Tech stack**: TypeScript, Elgato Stream Deck SDK v2 (`@elgato/streamdeck`), Zod (runtime validation), Rollup (bundler), @resvg/resvg-js, git-network-graph — versions: TS 5.3+, Stream Deck SDK 2.1+, Node 20+
- **Package manager**: npm | **Module system**: ES modules | **Runtime**: Node.js 20+
- **Plugin UUID**: `com.pedrofuentes.github-utilities`

## Commands

```bash
npm test -- <path>          # file-scoped tests (Vitest) — prefer
npm run lint -- <path>      # file-scoped lint (ESLint)
npm install | npm run build | npm test | npm run lint    # full suite
# tsc type-checks via `npm run build` (Rollup); no separate typecheck/format script — ESLint + tsconfig enforce style
# Stream Deck deploy/pack: see docs/DEVELOPMENT-WORKFLOW.md (streamdeck link/restart/validate/pack)
```
...
- **Formatter**: none separate — ESLint + tsconfig enforce style (tabs for indentation, double quotes). **Linter**: ESLint v9 (flat config, `eslint src`) with `@eslint/js` + `typescript-eslint` recommended — fix all warnings.
- Conventions: TypeScript only; ES module imports use explicit `.js` extensions; explicit return types on all exported functions; JSDoc on all exports; no unused vars/params (compiler-enforced); settings interfaces include a `[key: string]: JsonValue` index signature; new actions extend `BaseGitHubAction`.
...
- Run `npm test && npm run lint` before PR; invoke Sentinel before merge
...
**Triggers**: adding/removing dependencies · CI/CD or release automation changes · public API changes · architecture decisions · env vars/secrets · external network services · pushing commits or branches to the remote (default is **local-only** — never push without explicit user approval) · bumping the version in `package.json`/`manifest.json`
...
Auth/crypto/PII · DB migrations · AGENTS.md/SENTINEL.md changes · production deploys · 🔴 CRITICAL findings · 5× Sentinel rejections · deployment pipeline setup · credentials rotation · **releasing/packaging the plugin (`streamdeck pack`) or publishing to the Elgato Marketplace without the user first confirming a successful test on physical Stream Deck hardware**
...
- **Project (Stream Deck)**: hand-edit anything in `release/` (build output) · encode button SVGs as base64 or with a `charset` (only `encodeURIComponent` renders on hardware) · use nested `<svg>` elements (renderer-unsupported — use `<g transform>`) · log GitHub tokens in plaintext (use `maskToken()`) · include file extensions in `manifest.json` image paths · auto-push to the remote without explicit user approval
...
| [`content/CONTENT-GUIDE.md`](./content/CONTENT-GUIDE.md) | Updating the Elgato Marketplace listing |
...
## Template Collaboration (Stream Deck ecosystem)

This plugin participates in the **stream-deck-template** knowledge-sharing ecosystem
(https://github.com/pedrofuentes/stream-deck-template). Hard-won Stream Deck learnings
(SVG rendering, manifest quirks, Property Inspector patterns, polling, testing) are shared
across all of the author's plugins.

- **Before major work**: read the consolidated ecosystem knowledge at
  `https://raw.githubusercontent.com/pedrofuentes/stream-deck-template/main/LEARNINGS.md`.
  This *remote ecosystem* log is distinct from the local [`LEARNINGS.md`](./LEARNINGS.md),
  which records Sentinel rejection-pattern memory for this repo.
- **After significant work**: proactively offer to contribute new findings back to
  `contributions/github-utilities.md` in the template repo.
- Local companion guides mirror the ecosystem's testing & UI guides in
  [`.github/`](./.github/).
