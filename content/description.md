<!-- 
  Marketplace Description — Stream Deck GitHub Utilities
  Last Updated: v2.2.4
  Character Limit: 4,000
  Current Count: ~4,000 characters
-->

# GitHub Utilities for Stream Deck

**Your GitHub dashboard, one glance away.** Monitor repositories, track CI/CD pipelines, review PRs, and visualize branch networks — all from your Stream Deck and Stream Deck+.

14 actions. Full Stream Deck+ encoder support. Touch strip visualizations. No browser tab required.

---

## 🔢 Repo Stats

Turn any key into a live repository dashboard. Pick a stat, pick a repo — done.

- **10 stat types** — Stars, Issues, Forks, Watchers, PRs, Language, Size, License, Branch, Visibility
- **Short press** cycles stats on the fly; **long press** opens GitHub in your browser
- **SD+** — rotate the dial to cycle stats, tap the strip to refresh, see sparkline trends on the touch strip

---

## ⚙️ Workflow Status

Monitor GitHub Actions workflows and deployments in real time.

- **14 status states** — success, failure, running, queued, deploying, and more — each with its own color and icon
- **Deployment tracking** — active deployments show environment name with animated indicator
- **Run duration** displayed alongside status
- **SD+** — browse recent runs with the dial, long-touch to dispatch a workflow

---

## 👀 PR Review Queue

See how many pull requests are waiting for your review.

- **Urgency gradient** — blue (1 PR) → amber (3) → red (5+)
- **SD+** — hero count on the touch strip with repo context

---

## 🔀 PR Counter & 🐛 Issue Counter

Dedicated counters for pull requests and issues — accurate numbers at a glance.

- **State filter** — open, closed, or all

---

## 📦 Release Monitor

Track the latest release for any repository.

- **Version tag** with relative time (e.g., "v2.3.1 — 3d ago")

---

## 📊 Commit Activity

Show recent commit counts — last 24 hours, 7 days, or 30 days.

---

## 🌿 Branch Comparison

See how branches diverge with ahead/behind counts and color-coded status.

---

## 🌐 Branch Network *(SD+ only)*

A metro-map style git branch diagram rendered live on the touch strip.

- Scroll horizontally and vertically with the dial

---

## 🟩 Contribution Heatmap *(SD+ only)*

Your 52-week GitHub contribution grid on the touch strip — just like your profile page.

- **Profile mode** — contributions across all repos (via GraphQL)
- **Repo mode** — commit activity for a single repository

---

## 🛡️ Security Health

Dependabot alert summary with an A–F letter grade.

- **Arc gauge** on the SD+ touch strip with severity breakdown

---

## 📡 Fleet Monitor

Compact repo health dashboard — place 4 across for a fleet view.

- Workflow status + open PR count + commit sparkline in one key
- **SD+** — compact dashboard with trend visualization on the touch strip

---

## 💬 Discussions Monitor

Track discussions on any repository.

- **Total discussions** and **answered count** at a glance
- Auto-refresh with configurable interval
- Press to open discussions page

---

## 📊 Projects V2 Board

Monitor GitHub Projects V2 boards.

- **Project count** and **item totals** on the button
- Track project progress from your Stream Deck
- Press to open project board

---

## 🎛️ Stream Deck+ Everywhere

Every action supports the encoder. Rotate dials to cycle data, tap to refresh, push to open GitHub. Multiple instances of the same action stay in sync — refresh one and they all update. Visualizations include sparklines, arc gauges, heatmaps, and more.

---

## 🎯 Smart Property Inspector

- **One-time token setup** — enter your PAT once, shared across all buttons
- **Searchable dropdowns** — type to filter repos, workflows, branches, environments
- **Cascading filters** — selecting a repo loads its workflows and branches automatically

---

## 📋 Requirements

- Stream Deck software 6.9+ · macOS 13+ or Windows 10+
- GitHub Personal Access Token (fine-grained recommended) with Metadata, Actions, Deployments, Pull Requests, Issues, and Contents (all read)

---

Built with the official Elgato Stream Deck SDK. Open source under the MIT License.
